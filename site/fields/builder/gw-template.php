<div class="structure<?php e($field->readonly(), ' structure-readonly') ?> builder<?php e($field->readonly(), ' builder-readonly') ?>"
  ...
  <div class="builder-add-content-wrapper">
    <? if (count($field->fieldsets) > 1): ?>
      <div id="addButton" class="builder-add-content-button" title="Add Content"></div>
      <div class="builder-add-buttons">
        <?php foreach ($field->fieldsets as $fieldsetName => $fieldset):
          $fieldset = $field->fieldset($fieldsetName);
        ?>
          <a data-quickform href="#" data-href="<?= purl($field->page, 'field/' . $field->name . '/builder/add?fieldset=' . $fieldsetName) ?>">Add <?= $fieldset->label ?></a>
        <?php endforeach ?>
      </div>
    <? else: ?>
      <div class="builder-add-buttons visible">
        <?php foreach ($field->fieldsets as $fieldsetName => $fieldset):
          $fieldset = $field->fieldset($fieldsetName);
        ?>
          <a class="builder-add-content-button" data-quickform href="#" data-href="<?= purl($field->page, 'field/' . $field->name . '/builder/add?fieldset=' . $fieldsetName) ?>">Add <?= $fieldset->label ?></a>
        <?php endforeach ?>
      </div>
    <? endif; ?>
  </div>
